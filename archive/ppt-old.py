...
def main(argv):
    ...
    sheet1_raw = pd.read_excel(f'./{date}.xlsx', sheet_name=0, header=None)
    sheet2_raw = pd.read_excel(f'./{date}.xlsx', sheet_name=1, header=None)
    ...
    # 第二页ppt
    market = sheet1_raw.iat[2, 3]
    rating = sheet1_raw.iat[2, 2]
    all_rating = sheet1_raw.iat[4, 2]
    p_market: object = sheet1_raw.iat[1, 3]
    p_rating = sheet1_raw.iat[1, 2]
    p_all_rating = sheet1_raw.iat[3, 2]
    p_play = sheet2_raw.iat[4, 5]
    play = sheet2_raw.iat[4, 7]
    p_nplay = sheet2_raw.iat[5, 5]
    nplay = sheet2_raw.iat[5, 7]
    p_arrive = sheet1_raw.iat[1, 4]
    arrive = sheet1_raw.iat[2, 4]
    p_fidelity = sheet1_raw.iat[1, 5]
    fidelity = sheet1_raw.iat[2, 5]
    all_rating = sheet1_raw.iat[4, 2]
    p_all_rating = sheet1_raw.iat[3, 2]
    ...
    df = pd.concat([df, goodBye], ignore_index=True)
    ...
    midnight_matches = df_copy.loc[df_copy['单位 >>'].isin(['1900-01-01 00:00:00', '00:00:00'])].index
    hour_00 = midnight_matches[0] if len(midnight_matches) > 0 else len(df_copy)
